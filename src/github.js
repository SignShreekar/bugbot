...
function makeBranchName(priority, originalMessage) {
  const slug = originalMessage
    .replace(/<@[A-Z0-9]+>/g, '')
    ...
    .join('-')
    .toLowerCase();
  return `bugbot/${priority.toLowerCase()}-${slug}-${Date.now()}`;
}
...
async function createFixAndPR({ priority, repo, rca, affectedFiles, fixPlan, fileContents, originalMessage }) {
  ...
  // Create fix branch
  const branchName = makeBranchName(priority, originalMessage);
  ...
  // Build PR body ourselves — never embed long text inside JSON
  const prBody = [
    '## Summary',
    '',
    `- Automated fix for: ${originalMessage.slice(0, 200)}`,
    '',
    '## Root Cause',
    '',
    rca,
    ...
    '## Test Plan',
    '',
    '- [ ] Verify the reported bug is no longer reproducible',
